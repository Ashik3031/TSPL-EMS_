...
interface CelebrationPopupProps {
  ...
  onClose: () => void;
  // bgMusicUrl no longer used as default track, but kept for compatibility
  bgMusicUrl?: string;
  musicVolume?: number;
  duckedVolume?: number;
...
export default function CelebrationPopup({
  ...
  data,
  onClose,
  bgMusicUrl = '', // ✅ no default Rolex song anymore
  musicVolume = 0.25,
  duckedVolume = 0.08,
  countdownSeconds = 3,
  autoCloseMs = 12000,
  ...
  useEffect(() => {
    ...
    // 🔥 ONLY change: choose musicUrl based on teamId
    let musicUrlToUse = '';

    if (data.teamId === '6914828788fe1eb20016ddf3') {
      // Music 1 - Fuhad
      musicUrlToUse =
        'https://res.cloudinary.com/dugtxybef/video/upload/v1763205696/Scheme_-_Couper_Prod__Young_Maker_Malayalam_Rap_Song-_AudioTrimmer.com_yqyhci.mp3';
    } else if (data.teamId === '6914828788fe1eb20016ddf5') {
      // Music 2 - Andhan
      musicUrlToUse =
        'https://res.cloudinary.com/dugtxybef/video/upload/v1763207305/JAILER_-_Mathew_Theme_Superstar_Rajinikanth_Mohanlal_Sun_Pictures_Anirudh_Nelson-_AudioTrimmer.com_qafx7x.mp3';
    } else if (data.teamId === '6914828788fe1eb20016ddf7') {
      // Music 3 - Vimal
      musicUrlToUse =
        'https://res.cloudinary.com/dugtxybef/video/upload/v1763207785/KGF_Climax_Drums_BGM_-_Original_Soundtrack_-_Yash_Ravi_Barsur_-_KGF_Climax_BGM_Ringtone_lmngds.mp3';
    }

    // Debug (optional)
    // console.log('teamId:', data.teamId, 'musicUrlToUse:', musicUrlToUse);
